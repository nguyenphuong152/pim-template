<template>
  <v-navigation-drawer permanent width="100%" right absolutes floating>
          ...
          <div>
            <v-row>
              ...
              <v-col cols="1">
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon color="green" size="18" v-bind="attrs" v-on="on"
    ...
    <v-col v-for="(item, id) in getListTemplate" :key="id" cols="11">
      <v-card color="#f0f8ff">
        <div class="d-flex flex-no-wrap justify-space-between">
          <div>
            <v-row>
              <v-col class="text-start pl-8">
                ...
                <br />
                <span class="font-weight-thin">0 product in use</span>
              </v-col>
              <v-col cols="1">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
...
export default {
  ...
  computed: {
    getListTemplate() {
      return this.$store.getters.listTemplate;
    },
  },
  created() {
    console.log("render ", this.getListTemplate);
  },
};
</script>
