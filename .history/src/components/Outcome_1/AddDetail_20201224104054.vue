    ...
    >
      ...
      <v-expansion-panel-content>
        ...
        >
          ...
          <v-col>
            <v-text-field
              v-model="option[idCount + 1]"
              hint="Ex:selection 1"
            ></v-text-field>
          </v-col>
        </v-row>
        <v-row v-if="countSelection != ''">
          <v-spacer></v-spacer>
          <v-btn
            @click="previewSelections(countSelection)"
            outlined
            color="green"
            style="text-transform:none"
            >Preview</v-btn
...
export default {
  ...
  methods: {
    ...
    previewSelections(numb) {
      for (var i = 0; i < numb; i++) {
        this.listNameSelection.push(this.$options[i]);
      }
      console.log(this.listNameSelection);
    },
  },
};
</script>
